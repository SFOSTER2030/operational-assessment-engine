...
export interface RoiProjection {
    ...
    baselineTotalMonthlyCoat: number;
...
export function projectRoi(
    ...
    const monthlyDeploymentCost =
      deployment.monthlyInfrastructureUsd + deployment.monthlyMaintenanceUsd;

    const netMonthlySavings = laborSavings + errorSavings - monthlyDeploymentCost;
    const annualSavings = netMonthlySavings * 12;

    const totalInitialInvestment =
      deployment.initialSetupUsd + deployment.trainingAndOnboardingUsd;
    ...
    return {
          ...
          baselineTotalMonthlyCoat: baselineTotal,
